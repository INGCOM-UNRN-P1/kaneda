"""Regresión de KANEDA-D0301: `#if 0` no es código activo.

Tree-Sitter no ejecuta el preprocesador, así que parsea el contenido de un
`#if 0` como si estuviera vivo. Sin enmascararlo, una llamada peligrosa desactivada se auditaba y el
...
def test_el_cuerpo_del_if_0_se_blanquea():
    ...
    assert "gets" not in enmascarado
