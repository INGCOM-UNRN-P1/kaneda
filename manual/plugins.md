---
title: "Guía de Extensión y Creación de Plugins: kaneda"
subtitle: "Manual de integración, desarrollo de extensiones y uso de la API Python de kaneda"
author: "Cátedra de Algoritmos y Programación"
date: "2026-08-31"
---

(manual-kaneda-plugins)=
# Guía de Extensión y Plugins: kaneda

````{abstract}
Esta guía técnica detalla cómo desarrollar extensiones, crear nuevos plugins e integrar programáticamente **`kaneda`** en herramientas de evaluación, entornos de integración continua (CI/CD) o scripts docentes.
````

---

(manual-kaneda-plugins-arquitectura)=
## 1. Arquitectura de Extensión

`kaneda` provee una arquitectura modular desacoplada basada en puntos de entrada (Entry Points) estándar de Python (`[project.entry-points]`) o interfaces de inyección funcional:

- **Mecanismo de Extensión Principal**: `Extensiones y Reglas Personalizadas para kaneda`.
- **Punto de Entrada Oficial**: `kaneda.plugins`.
- **Formato de Comunicación**: Estructuras de datos serializables JSON / Pydantic models.

---

(manual-kaneda-plugins-tutorial)=
...
class CustomPlugin:
    """Plugin de extensión para kaneda."""
    name = "custom_rule"
...
Para que `kaneda` descubra y cargue automáticamente tu plugin, agregalo en tu `pyproject.toml`:

````{code-block} toml
[project.entry-points."kaneda.plugins"]
mi_plugin = "mi_paquete.modulo:MiPlugin"
````

### Paso 3: Instalar y Verificar el Plugin

Instalá tu extensión en modo editable y comprobá que `kaneda` la reconozca:
...
(manual-kaneda-plugins-sdk)=
## 3. Conexión Programática mediante la API Python

Podés importar y ejecutar `kaneda` directamente desde scripts de Python sin invocar subprocesos:
...
resultado = kaneda.ejecutar_analisis(
...
(manual-kaneda-plugins-ci)=
## 4. Integración en Pipelines de CI/CD (GitHub Actions / GitLab CI)

Podés integrar `kaneda` en tus flujos automatizados de Git para bloquear entregas que no cumplan los requisitos de cátedra:
...
jobs:
  auditoria:
    ...
    steps:
      ...
      - name: Instalar kaneda
        run: pip install -e ./kaneda
        
      - name: Ejecutar Auditoría
        run: kaneda check src/ include/ --json > reporte.json
````

---

(manual-kaneda-plugins-ejercicios)=
